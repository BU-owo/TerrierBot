...
- **ruleCog.py** (`community.rule`, `"Rule"`) — static, 1-indexed server rule list; look up one by number or list them all. No permission gating. `=rule [number]` / `/rule [number]` (autocomplete on `number`, previewing each rule's text). **In defaultCogs.**
- **squadPingCog.py** (`community.squadPing`, `"SquadPing"`) — roleless, self-service opt-in ping lists (JSON-backed, `data/softpings.json`), fixed hybrid command set (no per-list dynamic registration). `=squadpingcreate <name> <description>`, `=squadpinglist`, `=squadpingmanage <add|remove> <name>` (autocomplete on `name`), `=squadping <name>` (autocomplete on `name`, 1-per-30s per-user cooldown) — all hybrid. **In defaultCogs.**
...
- **logConfig.py** — **not a cog** (no `setup()`/`add_cog`), shared utility module: channel-ID constants, `MOD_ROLE_ID`, `MAIN_GUILD_ID`, colors, formatting helpers, in-memory cross-cog suppression registries, and the JSON-backed `register_queue_item()`/`resolve_queue_item()`/`get_stale_queue_items()` mod-queue reminder tracking (see modQueueReminderCog.py below). Imported by nearly every logging/moderation cog.
...
- **modQueueReminderCog.py** (`logging.modQueueReminder`, `"ModQueueReminder"`) — periodic (5 min) scan over logConfig.py's mod-queue item registry; pings the mod role once, replying to the original message, for any actionable mod-queue item (warn appeals, ban appeals, politics applications) still undecided an hour after being posted. No commands. **In defaultCogs.**
...
- **banCog.py** (`moderation.ban`, `"Ban"`) — ban/unban, temp-ban with auto-expiry, DM-based appeal intake, public ban announcements, optional server-wide message purge (`delete_history`, up to 7d — rule-8 bans default to a 4h purge if not overridden; the resulting Discord-native bulk delete is picked up and logged generically by messageLogCog's `on_raw_bulk_message_delete`, same as any other bulk delete). `duration`/`delete_history` share the same `NdNhNmNs` shorthand and both sit as bare optional positional args for `=ban`, so prefix invocations must write the latter as `purge:3d` to disambiguate which slot it fills — `/ban` has no such ambiguity (separate labeled inputs), so it takes a plain duration there. `=ban <member> [rule][duration][purge:Xd][reason]` / hybrid, `=unban <user_id>` / hybrid. **In defaultCogs.**
- **deleteUserDataCog.py** (`moderation.deleteUserData`, `"DeleteUserData"`) — owner-only, fully transparent (not hidden, invoking message not deleted, result posted publicly + to mod-log) permanent erase of a user's records across every data store: shelve keys (birthdays, lockins, hardmutes, roleboost, unserious, starboard, positivity's recent-selection cooldown, mod-tracker counts), `casedb.sqlite3`, `warnings.db`, `tempbans.json`, `modvotes.json` (cast votes only), `softpings.json` (list membership). Button-confirmed before anything is deleted. `=deletealluserdata <user_id>` (prefix, owner-only). **In defaultCogs.**
...
- **modTrackerCog.py** (`moderation.modTracker`, `"ModTracker"`) — owner-only: DMs a report of each mod-role member's logged action counts (retroactive, from `casedb.sqlite3`/`warnings.db`) and message activity split between the mod/log category and the rest of the server (live-tracked forward-only in shelve key `modtracker_message_counts`). Excludes the bot's own account (auto-unban etc. isn't moderator activity). `=modtracker` (owner-only prefix). **In defaultCogs.**
...
- **purgeCog.py** (`moderation.purge`, `"Purge"`) — bulk delete with purge attribution (feeds messageLogCog). `=purge <amount>`, `=purgeafter [target]`, `=purgeuser <user> <amount>` (deletes just that member's messages, scanning up to `PURGEUSER_SCAN_LIMIT`=500 back) (hybrid). **In defaultCogs.**
...
- **warnAppealCog.py** (`moderation.warnAppeal`, `"WarnAppeal"`) — appeal one's own active warning via select+modal, mod accept/reject buttons; the mod who issued the warning can Accept its appeal but is blocked from Rejecting it. `/warnappeal` (slash only). **In defaultCogs.**
...
| `modtracker_message_counts` | modTrackerCog | `{user_id_str: {"category": int, "rest": int}}` — mod-role members only, live-tracked forward from whenever the cog first loaded |
...
- **`data/mod_queue_reminders.json`** (gitignored, runtime-generated) — `{message_id_str: {channel_id, posted_ts, reminded: bool}}`. Written by logConfig.py's `register_queue_item()`/`resolve_queue_item()` (called from warnAppealCog.py, appealServerCog.py, joinPoliticsCog.py); read by modQueueReminderCog.py's periodic stale-item scan.
