...
class PoliticsApplicationModal(discord.ui.Modal, title="Politics Channel Application"):
    conduct_field = discord.ui.Label(
        ...
        description="Read the full conduct code above, then confirm below.",
        component=discord.ui.Select(
            placeholder="Select one",
            options=[
                discord.SelectOption(label="I agree", value=AGREE_VALUE),
                discord.SelectOption(label="I do not agree", value=DISAGREE_VALUE),
            ],
        ),
    )
    punishments_field = discord.ui.Label(
        text="Punishments",
        description="Read the full punishments policy above, then confirm below.",
        component=discord.ui.Select(
            placeholder="Select one",
            options=[
                discord.SelectOption(label="I understand", value=UNDERSTAND_VALUE),
                discord.SelectOption(label="I do not understand", value=NOT_UNDERSTAND_VALUE),
            ],
        ),
    )

    async def on_submit(self, interaction: discord.Interaction) -> None:
        conduct_select = self.conduct_field.component
        punishments_select = self.punishments_field.component
        assert isinstance(conduct_select, discord.ui.Select)
        assert isinstance(punishments_select, discord.ui.Select)

        conduct_answer = conduct_select.values[0]
        punishments_answer = punishments_select.values[0]

        if conduct_answer != AGREE_VALUE or punishments_answer != UNDERSTAND_VALUE:
        ...
        channel = interaction.client.get_channel(MOD_REVIEW_CHANNEL_ID)
...
class PoliticsApplicationStartView(discord.ui.View):
    ...
    async def start_application(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        await interaction.response.send_modal(PoliticsApplicationModal())
